...
void load_chat_list() {
    ...
    for(int i = 0; i < user_data.amount_of_chat; i++) {
        ...
        user_data.chat_array[i].chat_list_photo_event_box = gtk_box_new(GTK_ORIENTATION_HORIZONTAL, 0);
        gtk_widget_set_name(GTK_WIDGET(user_data.chat_array[i].chat_list_photo_event_box), "chat_small_box");
        gtk_widget_set_size_request(GTK_WIDGET(user_data.chat_array[i].chat_list_photo_event_box), 300, 70);
        gtk_container_add(GTK_CONTAINER(user_data.chat_array[i].chat_button), user_data.chat_array[i].chat_list_photo_event_box);
        ...
        gtk_box_pack_start(GTK_BOX(user_data.chat_array[i].chat_list_photo_event_box), user_data.chat_array[i].chat_list_photo, FALSE, FALSE, 0);
        ...
        gtk_box_pack_start(GTK_BOX(user_data.chat_array[i].chat_list_photo_event_box), user_data.chat_array[i].chat_label_name, FALSE, FALSE, 0);

        GtkWidget *chat_id = gtk_label_new(int_to_str(user_data.chat_array[i].chat_id));
        gtk_box_pack_start(GTK_BOX(user_data.chat_array[i].chat_list_photo_event_box), chat_id, FALSE, FALSE, 0);
