...
void start_screen() {
    ...
    main_data.login_box.styles = gtk_css_provider_new();
    gtk_css_provider_load_from_path(main_data.login_box.styles, "resource/styles/sign_in.css", NULL);
    gtk_style_context_add_provider_for_screen(gdk_screen_get_default(), GTK_STYLE_PROVIDER(main_data.login_box.styles), GTK_STYLE_PROVIDER_PRIORITY_USER);
