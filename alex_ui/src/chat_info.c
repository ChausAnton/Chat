...
void show_chat_info(GtkWidget *widget) {
    ...
    char *usr_amnt = mx_strjoin(int_to_str(user_data.chat_array[main_data.main_box.search_chat_id].count_users), " users");
    ...
    for(int i = 0; i < user_data.chat_array[main_data.main_box.search_chat_id].count_users; i++) {
        ...
        g_signal_connect(G_OBJECT(chat_info_avatar), "draw", G_CALLBACK(draw_user_avatar), path);
