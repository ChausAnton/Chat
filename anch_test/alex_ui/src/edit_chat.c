...
void show_edit_chat(GtkWidget *widget) {
    ...
    //Chat photo
    user_data.chat_array[main_data.main_box.search_chat_index].chat_settings_photo_event_box = gtk_event_box_new();
    //gtk_widget_set_name(GTK_WIDGET(user_data.chat_array[main_data.main_box.search_chat_index].chat_settings_photo_event_box), "user_settings_photo_event_box");
    gtk_box_pack_start(GTK_BOX(scrollable_box), user_data.chat_array[main_data.main_box.search_chat_index].chat_settings_photo_event_box, FALSE, FALSE, 0);
    ...
    g_signal_connect(G_OBJECT(user_data.chat_array[main_data.main_box.search_chat_index].chat_settings_avatar), "draw", G_CALLBACK(draw_user_settings_avatar), path);
    ...
    g_signal_connect(G_OBJECT(user_data.chat_array[main_data.main_box.search_chat_index].chat_settings_photo_event_box), "button_press_event", G_CALLBACK(change_chat_photo), scrollable_box);

    //Nickname
    GtkEntryBuffer *nickname = gtk_entry_buffer_new(user_data.chat_array[main_data.main_box.search_chat_index].chat_name, -1);
...
void save_edit_chat_changes(GtkWidget *widget, GdkEventButton *event, gpointer *data) {
    ...
    user_data.chat_array[main_data.main_box.search_chat_index].chat_name = strdup((char*)gtk_entry_buffer_get_text(gtk_entry_get_buffer(GTK_ENTRY(parent->next->data))));

    send(sock, user_data.chat_array[main_data.main_box.search_chat_index].chat_name , 
    strlen(user_data.chat_array[main_data.main_box.search_chat_index].chat_name), 0);
    recv(sock, s_message, 2000, 0);
    s_message = clear_client_message(s_message);

    gtk_label_set_text(GTK_LABEL(user_data.chat_array[main_data.main_box.search_chat_index].chat_label_name), user_data.chat_array[main_data.main_box.search_chat_index].chat_name);
    gtk_label_set_text(GTK_LABEL(main_data.main_box.chat_box_name_label), user_data.chat_array[main_data.main_box.search_chat_index].chat_name);
